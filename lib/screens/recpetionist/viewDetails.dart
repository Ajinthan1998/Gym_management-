...
class _ViewDetailsState extends State<ViewDetails> {
    ...
    try {
      ...
      if (paymentSnapshot.docs.isNotEmpty) {
        ...
        if (paymentData.containsKey('paymentAmount')) {
          setState(() {
            _paymentAmountController.text = paymentData['paymentAmount'].toString();
  ...
  void fetchPackageData() async {
    ...
    try {
      ...
      if (userData != null && userData['PackageName'] != null) {
        QuerySnapshot<
            Map<String, dynamic>> packageSnapshot = await FirebaseFirestore
            .instance
            .collection('packages')
        ...
        if (packageSnapshot.docs.isNotEmpty) {
          ...
          var duration = packageData['duration']; // New line to fetch package duration
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Package Name: ${userData['PackageName']}',style: TextStyle(
                  fontSize: 15,
                  color: Colors.white,
                ),),
                Text('Coach Name: ${userData['coachName']}',style: TextStyle(
                  fontSize: 15,
                  color: Colors.white,
                ),),
                Text('Selected Category: ${userData['SelectedCategory']}',style: TextStyle(
                  fontSize: 15,
                  color: Colors.white,
                ),),
                ...
                Text('Phone Number: ${userData['phone_no']}',
                  style: TextStyle(
                    fontSize: 15,
                    color: Colors.white,
                  ),),
                ...
                Text('Package Duration: $packageDuration months',
                  style: TextStyle(
                    fontSize: 15,
                    color: Colors.white,
                  ),),
                ...
                if (expiryDate.compareTo(currentDate) > 0 ) // Display only if payment is made and package duration is set
                  Column(
                    children: [
                      SizedBox(height: 20),
                      Text('Payment Amount: ${_paymentAmountController.text}',style: TextStyle(
                        fontSize: 15,
                        color: Colors.white,
                      ),
                      ),
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          enabledBorder: UnderlineInputBorder(
                            borderSide: BorderSide(color: Colors.white), // Change controller color
                          ),
                          focusedBorder: UnderlineInputBorder(
                            borderSide: BorderSide(color: Colors.white), // Change controller color when focused
                          ),
                        ),
                      ),
                      ElevatedButton(
  ...
  String calculateExpiryDate() {
    ...
    var expiryDate = currentDate.add(Duration(days: packageDuration.toInt() * 30)); // Assuming 30 days per month
